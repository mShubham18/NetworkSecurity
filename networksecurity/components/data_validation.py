...
class DataValidation:
    ...
    def initiate_data_validation(self)-> DataValidationArtifact:
        try:
            ...
            ## reading the data from train and test file
            ...
            """numerical_col = self.train_dataframe.select_dtypes(include=[np.number]).columns
            if len(numerical_col) == 0:
                error_message = f"Train dataframe does not contain any numerical columns. \n"
            numerical_col = self.test_dataframe.select_dtypes(include=[np.number]).columns
            if len(numerical_col) == 0:
